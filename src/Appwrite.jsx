...
export const getPersonalizedRecommendations = async (userId, limit = 10) => {
  try {
    ...
    const similarMovies = []; // Get movies that share genres with user's preferences
    const topGenres = Object.entries(genreScores)
      .sort((a, b) => b[1] - a[1])
      .slice(0, 5)
      .map(([genreId]) => parseInt(genreId)); // Filter out movies the user has already clicked
    ...
    if (topGenres.length > 0) {
      ...
      const filteredMovies = genreMovies.filter(
        (movie) => !clickedMovieIds.has(movie.$id),
      ...
      const scoredMovies = filteredMovies.map((movie) => {
        ...
        score += (movie.vote_average || 0) / 2; // Boost for recent/popular movies
    ...
    if (similarMovies.length < limit) {
      ...
      const trendingMovies_data = trending.filter(
        (m) =>
          !clickedMovieIds.has(m.$id) &&
          !similarMovies.some((sm) => sm.$id === m.$id),
      );
      similarMovies.push(
        ...trendingMovies_data.slice(0, limit - similarMovies.length),
      );
    }
...
export const getCollaborativeRecommendations = async (userId, limit = 10) => {
  try {
    ...
    const userMovieIds = userClicks.map((c) => c.movieId); // Find other users who clicked similar movies
    const otherUsersInteractions = await database.listDocuments(
      DATABASE_ID,
      USER_INTERACTIONS_COLLECTION_ID,
      [Query.notEqual("userId", userId), Query.limit(100)],
    ...
    Object.entries(userSimilarity)
      ...
      .forEach(([similarUserId, data]) => {
        ...
        similarUserMovies.forEach((movieId) => {
          if (
            !clickedSet.has(movieId) &&
            !recommendedMovies.includes(movieId)
          ) {
...
// Bookmark functions
export const saveBookmark = async (userId, movieId, movieData) => {
  try {
    ...
    if (existing.documents.length === 0) {
      await database.createDocument(
        ...
        ID.unique(),
        {
          userId,
          movieId,
          title: movieData.title, // Flatten these attributes
...
export const isBookmarked = async (userId, movieId) => {
  try {
    const result = await database.listDocuments(
      DATABASE_ID,
      BOOKMARKS_COLLECTION_ID,
